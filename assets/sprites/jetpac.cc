...
Terrain* terrain;// I'LL allow this but please no more unnecesary global variables
...
void SpawnObject(Object* object,int frameCounter);
...
void ResetShipComponentPositions(Ship* ship);
void UpdateShip(Ship* ship, Player* player, int frameCounter);
...
void LandingShip(Ship* ship, int frameCounter);
void InitShipAtTop(Ship* ship);
//HUD
...
// FOR THE ENEMIES
void InitEnemy(Enemy** enemies, int kNenemys);
...
//Enemies collision
void EnemyCollides(Enemy* enemies, int kNenemys, Player* player,bool iscollidingright,bool iscollidingleft,bool iscollidingdown,bool iscollidingup);
  ...
  while (!esat::IsSpecialKeyDown(esat::kSpecialKey_Escape) && esat::WindowIsOpened()){
    ...
    switch (gm.state){
      ...
      case KMAIN_LOOP:{
        ...
        //SHIP
        UpdateShip(&ship, &player, frameCounter); // ALGO EN UPDATE SHIP ROMPE TODO
        if(reached_top){// THIS SHOULD GO INSIDE A FUNCTION
          LandingShip(&ship,frameCounter);
        }
        // OBJECTS
        SpawnObject(object,frameCounter);
        UpdateObjects(object);
        ObjectInteraction(object,&player);
        PickComponents(&player,&ship);
        UpdateComponents(&ship,&player);
        // ENEMIES
        EnemyGeneration(enemies, kNenemys);
        EnemyAnimation(enemies, kNenemys);
        EnemyCollides(enemies, kNenemys, &player,iscollidingright,iscollidingleft,iscollidingdown,iscollidingup);
      ...
      case KNEXT_LEVEL:{
        // CLEAR SCREEN AND LOAD NEXT LEVEL 
        break;
      }
      case KTAKEOFF:{
      ...
      case KNEXTPLAYER:{
        // CHECK IF THERE IS ANOTHER PLAYER AND THEN LOAD HIS GAME
        break;
      }
      case KMENU:{
...
void MenuLogic(int frame_counter){
  ...
  if(esat::IsKeyDown('5') && gm.num_players!=0){
    gm.state = KMAIN_LOOP;
...
void readLevelFromFile(Level* level){
  ...
  stage = level->level_stage ;// this will be the offset to know wich part of the file we need to read
  ...
  fseek(file, level->level_stage * sizeof(Level), SEEK_SET);
...
void PlayerMovement(Player* player){
  ...
  if( player->is_holding_object == true){
    if( player->object->transform.x>=(kWindowWidth/2 +150)  &&  player->object->transform.x<=(kWindowWidth/2 +100 + 64) ){
  ...
  if( player->is_holding_component == true){
    if( player->component->transform.x>=(kWindowWidth/2 + 125)  &&  player->component->transform.x<=(kWindowWidth/2 +125 + 64) ){// >ship pos , <pos + width
      // let go off the object is you pass over the ship
      player->is_holding_component = false;
...
bool IsCollidingDown(esat::SpriteTransform player, esat::Vec2 WandH, Terrain* platform){
  ...
  esat::Vec2 P4 = {player.x + WandH.x - (WandH.x/5), player.y + WandH.y}; // Fourth point of the player 
...
/*
   ____   ____ _  __ _    _ _____    _____  
  |  __ |_   _| |/ /| |  | |  __ \  | ____| 
  | |__)  | | | ' / | |  | | |__) | | |__   
  |  ___/ | | |  <  | |  | |  ___/  |___ \  
  | |    _| |_| . \ | |__| | |      ___) | 
...
void SpawnObject(Object* object, int frameCounter){
  ...
  // count the frames when the fuel hasnt been on display
  if(!(object->is_active)){
  ...
  if(gm.no_objects_displayed && gm.frames_Since_Object_Spawn >= 60 * 10  ){
...
void InitShip(Ship* ship){
  if(ship != NULL){ //Only execute if ship exists
    ...
    //We need a variable so we can add distance between the components
    ...
    if(ship->component!=NULL){
      ...
      //Rocket 2 sprite component loading from file a_rocket[4-6].png

      //Rocket 3 sprite component loading from file a_rocket[7-9].png

      //Next components are the fire animation sprites. 
      ...
      for(int i = 0; i < 3; i++){
        ...
        //Y position: Y coords of terrain (height) +=64 (ship_height)
        //Every component in Y with 64px offset (64px = sprite pixel height)
        (ship->component+i)->transform.y = kWindowHeight - ((terrain)->height) - (64 + component_height);
        component_height += 64; //Add 64 for the next loop for Y coords adjustment
      ...
      //Elements 3 and 4 are for fire sprites
      for(int i = 3; i < 5; i++){
        esat::SpriteTransformInit(&(ship->component+i)->transform); //Init transform component
        (ship->component+i)->transform.x = kWindowWidth/2 + 125; //X = Half-screen + 125px
...
void InitShipAtTop(Ship* ship){
  ...
  for(int i = 0; i < 4; i++){
    (ship->component + i)->transform.x = kWindowWidth / 2 + 125;
    (ship->component + i)->transform.y = 0 + distancebetweenpieces;
    (ship->component + i)->is_conected = true;
    distancebetweenpieces -= 64;
  }
}

// To reset the component positions but in a function so we can call it whenever we need it instead of writing it again.
void ResetShipComponentPositions(Ship* ship){
  (ship->component + 0)->transform.x = kWindowWidth / 2 + 125;
      (ship->component + 0)->transform.y = kWindowHeight - terrain->height - 64;
      // Middle part on the patform
      (ship->component + 1)->transform.x = 450 + 30;
      (ship->component + 1)->transform.y = 400 - 64;
      // Point placed on the other patform
      (ship->component + 2)->transform.x = 132 + 70;
      (ship->component + 2)->transform.y = 292 - 64;
}
...
void DrawShip(Ship ship, int frameCounter){
  ...
  if(ship.fuel_counter >= 6 && ((ship.component+3)->transform.y) < 670) {
...
void UpdateShip(Ship* ship, Player* player, int frameCounter){
 ...
 if((ship->component+0)->transform.y >= kWindowHeight - ((terrain)->height)-64 && reached_top == true){
   reached_top = false;
   //Set fuel_counter to 0 at the botttom of Y animation
   ship->fuel_counter = 0;
   ship->components_attached = 1;
   ResetShipComponentPositions(ship);
 }
 
 //Every 10fps Y +/-= 5px (up is -) if fuel_counter >= 6.
 if(frameCounter % 5 == 0 && ship->fuel_counter >= 6 && !reached_top){
...
bool ColisionRightRay(RayComponent* sub_rays,Enemy* enemies) {
  ...
  if ((Point1.x > Qoint1.x && Point2.x < Qoint2.x + 10) && 
          (Point1.y < Qoint2.y && Point2.y > Qoint1.y) && enemies->is_alive != 0){
    return true;
  } else {
    return false;
  }
        ...
        // if not every subray has finnished then keedp drawing
          for (int i = 0; i < KNumbersOfSubRays; i++) {
              if ((current->sub_rays + i)->is_active) {
                ...
                // the position of the player + an offset to make the bullet start in front of the player and not from within it + whatever the ray moved;
                /*
                  THE WAY I THINK CHECKING COLLISIONS WONT DESTROY THE REST OF THE FUNCTION <----
                  if(i== current->front_component){
                    while( enemies)
                      if(areThesesColliding((current->sub_rays + i),enemigo+j))
                        (current->sub_rays + i)->is_active = false;
                        (current->sub_rays + i)->distance = 0; // stop drawin the ray and adding new subrays to the stream
                        current->distance = KraySpeed * KNumbersOfSubRays; this will make the current ray to be removed
                        //i = KNumbersOfSubRays; // i need the remaining subrays fo finnished too. make sure no new subrays are added after collision
...
/************************************** ENEMY RELATED CODE *****************************************/
void InitEnemy(Enemy** enemies, int kNenemys){//INITIAlICE SOME ENEMY VALUES
...
bool EnemyIsCollidingUp(esat::Vec2 enemies,esat::Vec2 WandH, Terrain* platform){
  // Box colider from the player
  esat::Vec2 P1 = {enemies.x , enemies.y}; // first point from the sprite of the player
  esat::Vec2 P2 = {enemies.x + WandH.x , enemies.y}; // second point from the sprite of the player
  //esat::Vec2 P3 = {player.x, player.y + WandH.y};
  //esat::Vec2 P4 = {player.x + WandH.x, player.y + WandH.y};
  ...
  esat::Vec2 Q2 = {platform->transform.x + platform->width, platform->transform.y}; // second point from the platform
  esat::Vec2 Q3 = {platform->transform.x, platform->transform.y + platform->height}; // third point from the player
  //esat::Vec2 Q4 = {platform->transform.x + platform->width, platform->transform.y + platform->height};
...
bool EnemyIsCollidingRight(esat::Vec2 enemies,esat::Vec2 WandH, Terrain* platform){
  //esat::Vec2 P1 = {player.x, player.y};
  esat::Vec2 P2 = {enemies.x + WandH.x, enemies.y}; // second point from the player
  //esat::Vec2 P3 = {player.x, player.y + WandH.y};
  esat::Vec2 P4 = {enemies.x + WandH.x, enemies.y + WandH.y}; // fourth point from the player
...
bool EnemyIsCollidingLeft(esat::Vec2 enemies,esat::Vec2 WandH, Terrain* platform){
  ...
  //esat::Vec2 P4 = {player.x + WandH.x, player.y + WandH.y}; // fourth point from the player
...
bool EnemyIsCollidingDown(esat::Vec2 enemies,esat::Vec2 WandH, Terrain* platform){
  // Box colider from the player
  //esat::Vec2 P1 = {player.x, player.y}; // first point from the sprite of the player
  //esat::Vec2 P2 = {player.x + WandH.x, player.y}; // second point from the sprite of the player
  esat::Vec2 P3 = {enemies.x, enemies.y + WandH.y};
